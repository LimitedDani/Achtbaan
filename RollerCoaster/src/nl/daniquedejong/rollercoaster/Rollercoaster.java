...
public class Rollercoaster
{
	...
	public Rollercoaster(String Name, String runcommand, String runpermissions, String spawncommand, String spawnpermissions, HashMap<String, List<String>> spawnlist, HashMap<String, List<String>> runlist){
		this.name = Name;
	...
	public boolean checkIfSpawnIdExists(String id){
		return spawn.containsKey(id);
	}
	public boolean checkIfRunIdExists(String id){
		return run.containsKey(id);
	}
	...
	private void run(String function, int id){
		...
		if(functionName.equals("spawn")){
			...
			A1.setHelmet(new ItemStack(Material.valueOf(values[6].toUpperCase()), 1, (byte)Integer.parseInt(values[7])));
			A1.setVisible(false);
			A1.setBasePlate(false);
			A1.setGravity(false);
			A1.setCustomName(A1.getUniqueId().toString());

			//String command1 = String.format("entitydata @e[name=%s] {ArmorItems:[{},{},{},{id:\"%s\",Count:1b,Damage:%s}],HandItems:[{},{}]}", A1.getUniqueId().toString(), values[6].toLowerCase(), values[7]);
	...
	private void run__A(int id){
		...
		for(int i = 1; i < total+1; i++)
		{
...

			{
				...
				try {
					Method armorstandHandle = A2.getClass().getMethod("getHandle");
					armorstandHandle.setAccessible(true);

					Object nmsArmorStand = armorstandHandle.invoke(A2);

					Method armorstandSetLocation = nmsArmorStand.getClass().getMethod("setLocation",
							double.class, double.class, double.class, float.class, float.class);
					armorstandSetLocation.setAccessible(true);

					armorstandSetLocation.invoke(nmsArmorStand, A1.getLocation().getX() + x, A1.getLocation().getY(), A1.getLocation().getZ() + z, A1.getLocation().getYaw(), A1.getLocation().getPitch());
				} catch (Exception e) {
					e.printStackTrace();
				}
